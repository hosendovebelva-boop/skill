...
| A concept unfolding step-by-step (algorithm / data-structure / "how X works") | **Step-driven explainer** (build-once + mutate) | CSS transitions only fire if the DOM persists between steps — see [references/step-animations.md](references/step-animations.md) |
...
**Animate by mutation, not rebuild.** For any step-through / state-machine animation (algorithm walkthroughs, "how X works" explainers driven by prev/next/play), build the DOM once and *mutate elements in place* each step (`className` / `style` / `textContent` on stable ids). Never `container.innerHTML = render(state)` per step — that destroys and recreates every node, so CSS `transition` has nothing to tween from and the UI snaps. This defect is invisible in the source (the CSS looks right) and is the top reason hand-written explainer animations feel janky. Slide pointers via `left`, tween magnitudes via `height`, fade regions via `background`. See [references/step-animations.md](references/step-animations.md).

**Note-explainer genre specs (scoped — do not apply elsewhere).** When the artifact is a step-driven HTML explainer animation for notes (e.g. Obsidian vault algorithm / systems animations), two additional specs are mandatory: SVG connectors follow [references/connector-arrows.md](references/connector-arrows.md) (small filled-triangle heads, tips touching the target, staggered fan-in, `stroke-dashoffset` draw-in), and embedded code panels follow [references/code-panels.md](references/code-panels.md) (all code visible with zero scrolling, high-contrast active-line highlight on dark backgrounds, sentinel-based syntax highlighting). These are conventions of that genre only — landing pages, decks, posters, dashboards, and brand work choose their own systems.
...
| Make a step-through explainer animation feel smooth (not snap) | [references/step-animations.md](references/step-animations.md) |
| Draw connector arrows in a note-explainer animation (scoped to that genre) | [references/connector-arrows.md](references/connector-arrows.md) |
| Embed a code panel in a note-explainer animation (scoped to that genre) | [references/code-panels.md](references/code-panels.md) |
